...
ER_range = np.linspace(0.75, 0.87, 19)
    ...
    while error_count < 100:
        ...
        try:
            ...
            if list(encoded_msg_copy) != list(decoded_msg_eec):
                error_count += 1
                bit_errors += M_BITS/2 * (len(erasure_indices) + hard_errors)
                print(error_count)
            ...
            print(error_count)
        total_count += 1
    total_bits = total_count * TOTAL_LENGTH * M_BITS
    return bit_errors / total_bits


def save_data(erasure_rate, new_header=False):
    BER_final = run_simulation(erasure_rate)
    header = ['Info symbols', 'EEC symbols', 'Symbol erasure rate', 'Background rate', 'Bit error rate']
    data = [INFO_LENGTH, EEC_LENGTH, erasure_rate, BACKGROUND_RATE, BER_final]
